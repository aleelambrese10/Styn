...
namespace Styn.ApiService.Controllers
{
    ...
    public class ServiciosController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType(typeof(IEnumerable<ServiciosDTO>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<ServiciosDTO>>> GetAllServicioss()
        {
            // Obtiene todos los elementos a través del servicio
            var items = await _ServiciosService.GetAllAsync();
            return Ok(items); // Devuelve 200 OK con la lista de DTOs
        }

        [HttpGet("{id}")]
        [ProducesResponseType(typeof(ServiciosDTO), StatusCodes.Status200OK)]
        ...
        public async Task<ActionResult<ServiciosDTO>> GetServicios(int id)
        {
            ...
            var item = await _ServiciosService.GetByIdAsync(id);
            if (item == null)
            {
                // Si no se encuentra, devuelve 404 Not Found
                return NotFound($"No se encontró Servicios con ID {id}.");
            }
        ...
        [HttpPost]
        [ProducesResponseType(typeof(ServiciosDTO), StatusCodes.Status201Created)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<ServiciosDTO>> CreateServicios([FromBody] ServiciosDTO data)
        {
            ...
            var createdItem = await _ServiciosService.CreateAsync(data);
            // Devuelve 201 Created con la ubicación del nuevo recurso y el DTO creado
            return CreatedAtAction(nameof(GetServicios), new { id = createdItem.Id }, createdItem);
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> UpdateServicios(int id, [FromBody] ServiciosDTO data)
        {
            ...
            try
            {
                ...
                await _ServiciosService.UpdateAsync(data);
            ...
            catch (KeyNotFoundException) // Captura si el servicio indica que el item no existe
            {
                ...
                return NotFound($"No se encontró Servicios con ID {id} para actualizar.");
            }
        ...
        public async Task<IActionResult> DeleteServicios(int id)
        {
             try
            {
                ...
                await _ServiciosService.DeleteAsync(id);
                return NoContent(); // Devuelve 204 No Content si la eliminación fue exitosa
            }
            catch (KeyNotFoundException) // Captura si el servicio indica que el item no existe
            {
                 // Devuelve 404 Not Found si el elemento no se encontró
                return NotFound($"No se encontró Servicios con ID {id} para eliminar.");
            }
