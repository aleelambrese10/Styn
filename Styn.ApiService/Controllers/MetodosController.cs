...
namespace Styn.ApiService.Controllers
{
    ...
    public class MetodosController : ControllerBase
    {
        ...
        private readonly IMetodosService _MetodosService;

        public MetodosController(IMetodosService MetodosService)
        {
            // Inyecta la dependencia del servicio
            _MetodosService = MetodosService ?? throw new ArgumentNullException(nameof(MetodosService));
        }

        [HttpGet]
        [ProducesResponseType(typeof(IEnumerable<MetodosDTO>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<MetodosDTO>>> GetAllMetodoss()
        {
            // Obtiene todos los elementos a través del servicio
            var items = await _MetodosService.GetAllAsync();
            return Ok(items); // Devuelve 200 OK con la lista de DTOs
        }

        [HttpGet("{id}")]
        [ProducesResponseType(typeof(MetodosDTO), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<MetodosDTO>> GetMetodos(int id)
        {
            // Obtiene un elemento por ID a través del servicio
            var item = await _MetodosService.GetByIdAsync(id);
            if (item == null)
            {
                // Si no se encuentra, devuelve 404 Not Found
                return NotFound($"No se encontró Metodos con ID {id}.");
            }
        ...
        [HttpPost]
        [ProducesResponseType(typeof(MetodosDTO), StatusCodes.Status201Created)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<MetodosDTO>> CreateMetodos([FromBody] MetodosDTO data)
        {
            ...
            var createdItem = await _MetodosService.CreateAsync(data);
            // Devuelve 201 Created con la ubicación del nuevo recurso y el DTO creado
            return CreatedAtAction(nameof(GetMetodos), new { id = createdItem.Id }, createdItem);
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> UpdateMetodos(int id, [FromBody] MetodosDTO data)
        {
            ...
            try
            {
                ...
                await _MetodosService.UpdateAsync(data);
            ...
            catch (KeyNotFoundException) // Captura si el servicio indica que el item no existe
            {
                ...
                return NotFound($"No se encontró Metodos con ID {id} para actualizar.");
            }
        ...
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> DeleteMetodos(int id)
        {
             try
            {
                // Elimina el elemento a través del servicio
                await _MetodosService.DeleteAsync(id);
                return NoContent(); // Devuelve 204 No Content si la eliminación fue exitosa
            }
            catch (KeyNotFoundException) // Captura si el servicio indica que el item no existe
            {
                 // Devuelve 404 Not Found si el elemento no se encontró
                return NotFound($"No se encontró Metodos con ID {id} para eliminar.");
            }
