...
namespace Styn.ApiService.Controllers
{
    ...
    public class TorosController : ControllerBase
    {
        ...
        private readonly ITorosService _TorosService;

        public TorosController(ITorosService TorosService)
        {
            // Inyecta la dependencia del servicio
            _TorosService = TorosService ?? throw new ArgumentNullException(nameof(TorosService));
        }

        [HttpGet]
        [ProducesResponseType(typeof(IEnumerable<TorosDTO>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<TorosDTO>>> GetAllToross()
        {
            // Obtiene todos los elementos a través del servicio
            var items = await _TorosService.GetAllAsync();
            return Ok(items); // Devuelve 200 OK con la lista de DTOs
        }

        [HttpGet("{id}")]
        [ProducesResponseType(typeof(TorosDTO), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<TorosDTO>> GetToros(int id)
        {
            // Obtiene un elemento por ID a través del servicio
            var item = await _TorosService.GetByIdAsync(id);
            if (item == null)
            {
                // Si no se encuentra, devuelve 404 Not Found
                return NotFound($"No se encontró Toros con ID {id}.");
            }
        ...
        [HttpPost]
        [ProducesResponseType(typeof(TorosDTO), StatusCodes.Status201Created)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<TorosDTO>> CreateToros([FromBody] TorosDTO data)
        {
            ...
            var createdItem = await _TorosService.CreateAsync(data);
            // Devuelve 201 Created con la ubicación del nuevo recurso y el DTO creado
            return CreatedAtAction(nameof(GetToros), new { id = createdItem.Id }, createdItem);
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> UpdateToros(int id, [FromBody] TorosDTO data)
        {
            ...
            try
            {
                ...
                await _TorosService.UpdateAsync(data);
            ...
            catch (KeyNotFoundException) // Captura si el servicio indica que el item no existe
            {
                ...
                return NotFound($"No se encontró Toros con ID {id} para actualizar.");
            }
        ...
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> DeleteToros(int id)
        {
             try
            {
                // Elimina el elemento a través del servicio
                await _TorosService.DeleteAsync(id);
                return NoContent(); // Devuelve 204 No Content si la eliminación fue exitosa
            }
            catch (KeyNotFoundException) // Captura si el servicio indica que el item no existe
            {
                 // Devuelve 404 Not Found si el elemento no se encontró
                return NotFound($"No se encontró Toros con ID {id} para eliminar.");
            }
